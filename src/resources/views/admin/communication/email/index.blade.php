                                    ...
                                    <div class="d-flex align-items-center gap-1">
                                        @if(@$log?->message)
                                            ...
                                            <button data-log-id="{{ $log->id }}" 
                                                    ...
                                                    data-subject="{{ @$log?->message?->subject ? replaceContactVariables($log->contact, $log->message->subject) : translate("N/A")}}"  
                                        ...
                                        <button data-url = "{{route('admin.communication.email.delete',['id' => $log->id])}}"
                                                class="icon-btn btn-ghost btn-sm danger-soft circle text-danger delete-email-log" type="button" data-bs-toggle="modal" data-bs-target="#deleteEmailLog">
...
<div class="modal fade" id="updateLog" tabindex="-1" aria-labelledby="updateLog" aria-hidden="true">
    <div class="modal-dialog modal-md modal-dialog-centered ">
        <div class="modal-content">
            <form action="{{route('admin.communication.email.status.update')}}" method="POST" enctype="multipart/form-data">
				@csrf
...
<script>
	"use strict";
        ...
        $('.update-log').on('click', function() {
            ...
            const responseMessage = $(this).data('response_message');
            ...
            modal.find('textarea[name=response_message]').val(logId);
            modal.find('select[name=status]').val(logStatus).trigger('change');
            modal.find('.log-view').attr('href', '{{ route("admin.communication.email.show", ":id") }}'.replace(':id', logId));
